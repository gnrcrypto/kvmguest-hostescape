...
#define SHARED_BUF_SIZE (4096 * 16)  // Increased to 64KB

// Large contiguous memory pool for extensive sharing
#define LARGE_POOL_SIZE (1024 * 1024 * 16)  // 16MB
static void *g_large_pool = NULL;
static unsigned long g_large_pool_gpa = 0;
...
struct batch_hypercall {
    struct hypercall_args calls[16];
    int num_calls;
};

struct memory_map_request {
    unsigned long target_gpa;
    unsigned long size;
    int flags;
};
...
// New advanced IOCTLs
#define IOCTL_BATCH_HYPERCALL    0x101E
#define IOCTL_ALLOC_LARGE_POOL   0x101F
#define IOCTL_GET_LARGE_POOL_GPA 0x1020
#define IOCTL_MAP_MEMORY_RANGE   0x1021
#define IOCTL_PROBE_ADDRESS      0x1022
#define IOCTL_HYPERCALL_WITH_DMA 0x1023

MODULE_LICENSE("GPL");
MODULE_AUTHOR("KVM Escape Lab");
MODULE_DESCRIPTION("Advanced kernel module for KVM exploitation with extended memory sharing");
...
// Probe if an address is accessible
static int probe_address(unsigned long addr, int write_test) {
...
static long driver_ioctl(struct file *f, unsigned int cmd, unsigned long arg) {
    ...
    switch (cmd) {
        ...
        case IOCTL_BATCH_HYPERCALL: {
            struct batch_hypercall batch;
            if (copy_from_user(&batch, (void __user *)arg, sizeof(batch)))
                return -EFAULT;
            
            printk(KERN_INFO "%s: BATCH HYPERCALL with %d calls\n", DRIVER_NAME, batch.num_calls);
            
            for (int i = 0; i < batch.num_calls && i < 16; i++) {
                batch.calls[i].ret_value = do_hypercall(&batch.calls[i]);
            }
        ...
        case IOCTL_VIRT_TO_PHYS: {
            ...
            if (va >= TASK_SIZE) {
                ...
                printk(KERN_INFO "%s: Userspace virt 0x%lx -> GPA 0x%lx\n", 
                       DRIVER_NAME, va, pa);
            }
        ...
        case IOCTL_ALLOC_SHARED_BUF: {
            ...
            printk(KERN_INFO "%s: Allocated shared buffer @ kernel virt %px -> GPA 0x%lx (size=%d)\n", 
                   DRIVER_NAME, g_shared_buffer, g_shared_buffer_gpa, SHARED_BUF_SIZE);
        ...
        case IOCTL_ALLOC_LARGE_POOL: {
            if (g_large_pool) {
                vfree(g_large_pool);
            }
            
            printk(KERN_INFO "%s: Allocating large pool (%d MB)...\n", 
                   DRIVER_NAME, LARGE_POOL_SIZE / (1024*1024));
            
            g_large_pool = vmalloc(LARGE_POOL_SIZE);
            if (!g_large_pool) {
                printk(KERN_ERR "%s: Failed to allocate large pool\n", DRIVER_NAME);
                return -ENOMEM;
            }
            
            memset(g_large_pool, 0, LARGE_POOL_SIZE);
            
            // Get GPA of first page (for reference)
            g_large_pool_gpa = virt_to_phys(g_large_pool);
            
            printk(KERN_INFO "%s: Large pool allocated @ virt %px -> GPA base 0x%lx\n",
                   DRIVER_NAME, g_large_pool, g_large_pool_gpa);
            
            if (copy_to_user((void __user *)arg, &g_large_pool_gpa, sizeof(g_large_pool_gpa)))
                return -EFAULT;
            break;
        }

        case IOCTL_GET_LARGE_POOL_GPA: {
            if (!g_large_pool) {
                printk(KERN_ERR "%s: Large pool not allocated\n", DRIVER_NAME);
                return -EINVAL;
            }
        ...
        case IOCTL_PROBE_ADDRESS: {
            unsigned long probe_addr;
            int result;
            
            if (copy_from_user(&probe_addr, (void __user *)arg, sizeof(probe_addr)))
                return -EFAULT;
            
            result = probe_address(probe_addr, 0);
            printk(KERN_INFO "%s: Probed address 0x%lx: %s\n",
                   DRIVER_NAME, probe_addr, result ? "ACCESSIBLE" : "NOT ACCESSIBLE");
...
static int __init mod_init(void) {
    printk(KERN_INFO "%s: Init - Advanced memory sharing features\n", DRIVER_NAME);
    ...
    printk(KERN_INFO "%s: Enhanced features: large pools, batch hypercalls, address probing\n", DRIVER_NAME);
    return 0;
}
